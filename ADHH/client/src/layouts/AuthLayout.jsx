  ...
  return (
    <main className="auth-site" ref={scope}>
      <header className="site-header">
        <Link to="/login" className="site-logo" onClick={() => setMenuOpen(false)}>
        ...
        <nav className={`site-nav ${menuOpen ? "open" : ""}`}>
          <div className="nav-group">
            ...
            <div className="nav-menu">
              <Link to="/login" onClick={() => setMenuOpen(false)}>Approach</Link>
              <Link to="/signup" onClick={() => setMenuOpen(false)}>Admissions</Link>
              <Link to="/forgot-password" onClick={() => setMenuOpen(false)}>Family support</Link>
            </div>
          </div>
          <div className="nav-group">
            <button type="button">About</button>
            <div className="nav-menu">
              <Link to="/login" onClick={() => setMenuOpen(false)}>Company</Link>
              <Link to="/login" onClick={() => setMenuOpen(false)}>Newsroom</Link>
            </div>
          </div>
        </nav>
        <div className="site-actions">
          <Link className="text-link" to="/login">Sign in</Link>
          <Link className="pill-button dark" to="/signup">Get started</Link>
        </div>
      </header>
      <section className="auth-page">
        <div className="auth-brand">
          <div className="auth-hero-copy">
            ...
            <h1>{SCHOOL.name}</h1>
            <p>A focused operating system for student records, admissions, and role-aware academic workflows.</p>
      ...
      <footer className="site-footer">
        ...
        <div className="footer-links">
          <div><strong>Student Experience</strong><Link to="/login">Approach</Link><Link to="/signup">Admissions</Link><Link to="/forgot-password">Support</Link></div>
          <div><strong>School Office</strong><Link to="/login">Sign in</Link><Link to="/signup">Get started</Link></div>
